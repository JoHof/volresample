...
cdef void _resample_linear_multi(
    ...
    # Preserve depth-wise reuse when there are enough planes; expose rows
    # only for shallow volumes that would otherwise leave workers idle.
    cdef int row_blocks = 1
    cdef int rows_per_task = out_h
    if n_channels * out_d < omp_get_max_threads():
        row_blocks = out_h
        rows_per_task = 1
    cdef int total_tasks = n_channels * out_d * row_blocks
    cdef int plane, first_row
    ...
    for task_id in prange(total_tasks, schedule='static',
                          use_threads_if=n_channels * out_vol >= 2048):
        plane = task_id / row_blocks
        first_row = (task_id - plane * row_blocks) * rows_per_task
        ch = plane / out_d
        od = plane - ch * out_d
        ...
        for oh in range(first_row, first_row + rows_per_task):
            h0 = h0_arr[oh]
            h1 = h1_arr[oh]
            fh = fh_arr[oh]
            fh1 = fh1_arr[oh]
